        ...
        {plans.map((plan, idx) => {
          ...
          return (
            <motion.div 
              ...
              className={`bg-gradient-to-r from-sky-100/90 via-pink-100/90 via-orange-100/90 via-pink-100/90 to-sky-100/90 backdrop-blur-md rounded-3xl p-3 sm:p-3.5 py-2.5 sm:py-3 border flex flex-col justify-between space-y-2 relative transition-all duration-300 hover:scale-102 shadow-md ${
                isCurrentPlanActive 
            ...
            >
              <div className="space-y-1.5">
                ...
                <ul className="space-y-0.5 text-sm text-slate-800 font-medium pt-1 border-t border-pink-200/80">
                  {plan.features.slice(0, visibleFeaturesCount).map((feat, fIdx) => (
                    <motion.li 
                      ...
                      className="flex items-start justify-between gap-1.5 leading-snug cursor-pointer py-0.5 px-1.5 rounded-lg hover:bg-white/90 hover:shadow-xs transition-all group border border-transparent hover:border-pink-200"
              ...
              {planNameLower.includes('космос') ? (
                <button 
                  ...
                  className="w-full py-2.5 bg-gradient-to-r from-sky-400 via-pink-500 to-orange-400 text-white font-bold text-sm rounded-2xl shadow-md transition-all duration-200 hover:opacity-95 cursor-pointer active:scale-98 mt-2 flex items-center justify-center gap-2"
                >
                  <span>Связаться</span>
